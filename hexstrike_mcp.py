...
DEFAULT_REQUEST_TIMEOUT = 900  # 15 minutes default timeout for API requests (raised from 300s)
...
def setup_mcp_server(hexstrike_client: HexStrikeClient) -> FastMCP:
    ...
    @mcp.tool()
    def jsfinder_scan(target: str, cookie: str = "", deep: bool = False, is_js: bool = False) -> Dict[str, Any]:
        """
        Execute a JSFinder scan to discover URLs and subdomains from JavaScript files or HTML pages.

        Args:
            target: The target URL to scan (e.g., http://www.example.com)
            cookie: Optional cookie string for authenticated scanning
            deep: Enable deep mode to crawl and extract more JS-related endpoints
            is_js: True if target is a direct JavaScript file URL

        Returns:
            Structured scan results including discovered URLs, subdomains, JS files, and raw output
        """
        data = {
            "target": target,
            "cookie": cookie,
        ...
        logger.info(f"{HexStrikeColors.FIRE_RED}🔍 Initiating JSFinder scan: {target}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/jsfinder", data)

        if result.get("success"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ JSFinder scan completed successfully for {target}{HexStrikeColors.RESET}")

            parsed = result.get("parsed", {})
            urls_count = len(parsed.get("urls", []))
            subs_count = len(parsed.get("subdomains", []))
            js_count = len(parsed.get("javascript_files", []))

            logger.info(
                f"{HexStrikeColors.CYBER_ORANGE}📊 JSFinder Results | URLs={urls_count} | Subdomains={subs_count} | JS Files={js_count}{HexStrikeColors.RESET}"
            )

            recovery = result.get("recovery_info", {})
            if recovery.get("recovery_applied"):
                attempts = recovery.get("attempts_made", 1)
                logger.info(f"{HexStrikeColors.HIGHLIGHT_YELLOW}🛡️ Recovery applied: {attempts} attempts made{HexStrikeColors.RESET}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ JSFinder scan failed for {target}{HexStrikeColors.RESET}")
            if result.get("human_escalation"):
                logger.error(f"{HexStrikeColors.CRITICAL}⚠️ HUMAN ESCALATION REQUIRED{HexStrikeColors.RESET}")

        return result
    
    @mcp.tool()
    def auth_http_request(
        url: str,
        method: str = "GET",
        ...
        """
        对目标发起带自动认证态的 HTTP 请求。
        可选先触发 Playwright 登录，再自动携带 cookie/header 发请求。
        """
        login_result = None

        if login_first:
            login_result = hexstrike_client.safe_post("api/tools/playwright/auth/login", {
                "target_url": url,
        ...
        export_result = hexstrike_client.safe_post("api/tools/playwright/auth/export-cookie", {
            "target_url": url,
            "state_name": state_name
        })
        ...
        if not export_result.get("success"):
            return {
                ...
                "stage": "export_cookie",
                "login_result": login_result,
                "cookie_export": export_result
            }

        cookie_header = export_result.get("cookie_header", "") or ""
        auth_headers = export_result.get("auth_headers", {}) or {}
        ...
        logger.info(
            f"{HexStrikeColors.CYBER_ORANGE}🍪 Using exported auth | "
            f"cookie_header={cookie_header} | "
            f"target_cookie_names={export_result.get('target_cookie_names', [])} | "
            f"auth_headers={list(auth_headers.keys()) if isinstance(auth_headers, dict) else []}"
            f"{HexStrikeColors.RESET}"
        )

        request_result = hexstrike_client.safe_post("api/tools/http-framework", {
            "action": "request",
            "url": url,
            "method": method,
            "headers": headers
        ...
        overall_success = bool(request_result.get("success"))
        ...
        return {
            ...
            "used_cookie_header": cookie_header,
    ...
    def playwright_auth_export_cookie(
        ...
        """
        导出 Playwright 登录后的 Cookie Header，供后续工具复用。
        ...
        Returns:
            cookie_header, cookies, cookie_count 等信息
        """
        data = {
        ...
        logger.info(f"{HexStrikeColors.CYBER_ORANGE}🍪 Exporting Playwright auth cookies: {target_url}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/playwright/auth/export-cookie", data)

        if result.get("success"):
            logger.info(
                f"{HexStrikeColors.SUCCESS}✅ Cookie export success | "
                f"count={result.get('cookie_count', 0)} | "
                f"target_cookie_names={result.get('target_cookie_names', [])} | "
                f"cookie_header={result.get('cookie_header', '')}"
                f"{HexStrikeColors.RESET}"
            )
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Cookie export failed for {target_url}{HexStrikeColors.RESET}")

        return result
    
    @mcp.tool()
    def playwright_auth_login_and_export(
        target_url: str,
        login_success_url_pattern: str = "",
        success_selector: str = "",
        state_name: str = "",
        ...
        """
        使用 Playwright 进行人工登录，并在成功后导出可复用 Cookie Header。
        """
        login_result = hexstrike_client.safe_post("api/tools/playwright/auth/login", {
            "target_url": target_url,
            "login_success_url_pattern": login_success_url_pattern,
        ...
        logger.info(
            f"{HexStrikeColors.SUCCESS}✅ Login+Export completed | "
            f"cookie_header={export_result.get('cookie_header', '')} | "
            f"target_cookie_names={export_result.get('target_cookie_names', [])}"
            f"{HexStrikeColors.RESET}"
        )
    ...
    def playwright_auth_login(
        ...
        """
        使用 Playwright 进行人工辅助认证登录。
        优先复用旧状态，失效时弹出浏览器供人工完成验证码/登录，然后保存状态。

        Args:
            target_url: 登录页或受保护入口 URL
            login_success_url_pattern: 登录成功后的 URL 特征，例如 /dashboard
            success_selector: 登录成功后的页面元素选择器，例如 text=退出登录
            state_name: 可选，自定义状态文件名
            force_reauth: 是否强制重新认证
            timeout_seconds: 人工等待登录超时秒数

        Returns:
            登录状态结果与 storage state 文件信息
        """
        data = {
            "target_url": target_url,
            "login_success_url_pattern": login_success_url_pattern,
        ...
        logger.info(f"{HexStrikeColors.FIRE_RED}🔐 Starting Playwright auth login: {target_url}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/playwright/auth/login", data)

        if result.get("success"):
            if result.get("reused"):
                logger.info(f"{HexStrikeColors.SUCCESS}✅ Reused existing Playwright auth state for {target_url}{HexStrikeColors.RESET}")
            else:
                logger.info(f"{HexStrikeColors.SUCCESS}✅ Playwright manual auth completed for {target_url}{HexStrikeColors.RESET}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Playwright auth login failed for {target_url}{HexStrikeColors.RESET}")
    ...
    def playwright_auth_check(
        ...
        logger.info(f"{HexStrikeColors.CRIMSON}🔎 Checking Playwright auth state: {target_url}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/playwright/auth/check", data)

        if result.get("success") and result.get("valid"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Playwright auth state is valid for {target_url}{HexStrikeColors.RESET}")
        else:
            logger.warning(f"{HexStrikeColors.WARNING}⚠️ Playwright auth state is invalid for {target_url}{HexStrikeColors.RESET}")
    ...
    def playwright_auth_clear(
        ...
        logger.info(f"{HexStrikeColors.CYBER_ORANGE}🧹 Clearing Playwright auth state: {target_url}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/playwright/auth/clear", data)

        if result.get("success"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Playwright auth state cleared for {target_url}{HexStrikeColors.RESET}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Failed to clear Playwright auth state for {target_url}{HexStrikeColors.RESET}")
    ...
    def awvs_scan(
        target_url: str,
        scan_name: str = "hexstrike-awvs",
        awvs_url: Optional[str] = None,
        api_key: Optional[str] = None,
        scan_profile_id: Optional[str] = None
    ) -> dict:
        """
        启动 AWVS 漏洞扫描。
        当需要对指定的 Web 目标进行安全扫描时调用此工具。
        :param target_url: 必须提供，目标网站的完整 URL (如 http://example.com)
        :param scan_name: 扫描任务的名称，默认为 hexstrike-awvs
        :param awvs_url: AWVS 服务器地址，可选，后端已有默认配置
        :param api_key: AWVS API 密钥，可选，后端已有默认配置
        :param scan_profile_id: 扫描配置 ID，可选，后端已有默认配置
        """
        payload = {
            "target_url": target_url,
            "scan_name": scan_name,
        }
        # 只有当 AI 显式提供了这些参数时，才传给后端，否则让后端使用默认值
        if awvs_url: payload["awvs_url"] = awvs_url
        if api_key: payload["api_key"] = api_key
        if scan_profile_id: payload["scan_profile_id"] = scan_profile_id
        
        return hexstrike_client.safe_post("api/tools/awvs", payload)
    ...
    def awvs_status(
        ...
        """
        检查 AWVS 扫描任务的当前状态。
        当需要知道一个正在进行的 AWVS 扫描任务是否完成，或者查看其进度时调用此工具。
        :param scan_id: 必须提供，之前调用 awvs_scan 返回的扫描任务 ID
        """
        payload = {"scan_id": scan_id}
        if awvs_url: payload["awvs_url"] = awvs_url
        if api_key: payload["api_key"] = api_key
        
        return hexstrike_client.safe_post("api/tools/awvs/status", payload)


    @mcp.tool()
    def awvs_report(
        scan_id: str,
        template_id: str = "11111111-1111-1111-1111-111111111111", # AWVS 常见的默认模板ID (如 Developer 报告等)
        export_format: str = "pdf",
        awvs_url: Optional[str] = None,
        api_key: Optional[str] = None
    ) -> dict:
        """
        当扫描任务完成后，生成并下载一份 AWVS 扫描报告。
        :param scan_id: 必须提供，你要下载报告的扫描任务 ID
        :param template_id: 报告模板ID，默认是 11111111-1111-1111-1111-111111111111
        :param export_format: 导出的文件格式，默认为 pdf (也可尝试 html 等)
        """
        payload = {
            "scan_id": scan_id,
        ...
        return hexstrike_client.safe_post("api/tools/awvs/report", payload)
    ...
    def nessus_scan(
        ...
        scan_name: str = "hexstrike-nessus",
        ...
        """
        启动 Nessus 漏洞扫描。
        除非需要覆盖默认配置，否则无需提供 url、key、uuid 等参数。
        """
        # 处理 targets 格式
        formatted_targets = [targets] if isinstance(targets, str) else targets
        ...
        # 仅当传入了覆盖参数时，才添加到 payload 中
        if nessus_url: data["nessus_url"] = nessus_url
        if access_key: data["access_key"] = access_key
        if secret_key: data["secret_key"] = secret_key
        if uuid: data["uuid"] = uuid
        if policy_id: data["policy_id"] = policy_id

        return hexstrike_client.safe_post("api/tools/nessus", data)
    ...
    def nessus_status(
        ...
        return hexstrike_client.safe_post("api/tools/nessus/status", data)
    ...
    def nessus_report(
        ...
        """
        触发 Nessus 报告导出（文件由服务器端保存），支持格式: csv, html, pdf
        """
        data = {
            "scan_id": scan_id,
            "export_format": export_format,
        ...
        return hexstrike_client.safe_post("api/tools/nessus/report", data)
    ...
    def metasploit_run(
        module: str,
        options: Dict[str, Any] = None,   # Bug8修复: None 替代 {}
        run_type: str = "exploit"
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            options: Dictionary of module options (e.g. {"RHOSTS": "192.168.1.1", "LHOST": "10.0.0.1"})
            run_type: Execution command - "exploit" (default), "run" (alias), or "check" (safe check only)

        Returns:
            Metasploit execution results including spool output
        """
        if options is None:       # Bug8修复: 每次调用生成新 dict
            options = {}
        data = {
            "module": module,
            "options": options,
        ...
        logger.info(f"🚀 Starting Metasploit module: {module} [{run_type}]")
    ...
    @mcp.tool()
    def msf_search_module(keyword: str, search_type: str = "") -> Dict[str, Any]:
        """
        Search for Metasploit modules by keyword, CVE, or type.

        Args:
            keyword: Search keyword (e.g. "eternalblue", "CVE-2021-44228", "smb")
            search_type: Optional filter: exploit, auxiliary, post, payload
        ...
        logger.info(f"🔍 Searching Metasploit modules: {keyword}")
        result = hexstrike_client.safe_post("api/tools/msf_search", data)
        if result.get("success"):
            logger.info(f"✅ Metasploit search completed for: {keyword}")
        else:
            logger.error(f"❌ Metasploit search failed for: {keyword}")
        return result
    ...
    def autorecon_comprehensive(target: str, output_dir: str = "/tmp/autorecon",
                               ...
                               heartbeat: int = 60, timeout: int = 900,
                               additional_args: str = "") -> Dict[str, Any]:
        """
    ...
    def ghidra_analysis(binary: str, project_name: str = "hexstrike_analysis",
                       script_file: str = "", analysis_timeout: int = 1200,
    ...
    @mcp.tool()
    def httpx_probe_bulk(targets: str = "", target_file: str = "", ports: str = "", methods: str = "GET", status_code: str = "", content_length: bool = False, output_file: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute HTTPx for bulk HTTP probing with enhanced logging.
        ...
        logger.info(f"🌐 Starting HTTPx bulk probing")
        result = hexstrike_client.safe_post("api/tools/httpx", data)
        if result.get("success"):
            logger.info(f"✅ HTTPx bulk probing completed")
        else:
            logger.error(f"❌ HTTPx bulk probing failed")
        return result
